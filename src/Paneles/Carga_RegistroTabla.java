...
public class Carga_RegistroTabla extends JPanel {
	...
	public Carga_RegistroTabla(JFrame ola,String user,JPanel panelcontenedordalso) {
		...
		label.setIcon(new ImageIcon(Carga_RegistroTabla.class.getResource("/Imagenes/giphy.gif")));
		label.setHorizontalAlignment(SwingConstants.CENTER);
		label.setBounds(10, 116, 450, 156);
		panel.add(label);
		
		JLabel label_1 = new JLabel("Cargando Libreta");
		...
		Thread hilo=new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					if(con.profesor(user)==true){
						ola.setLocationRelativeTo(null);
						ola.setSize(800,600);
						
						new CambiaPanel(panelcontenedordalso, new Libreta(ola,user,panelcontenedordalso));
						
						
						
					}else{
						JOptionPane.showMessageDialog(null, "No eres profesor");
						ola.dispose();
					}
